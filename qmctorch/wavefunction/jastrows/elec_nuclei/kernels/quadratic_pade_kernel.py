...
class QuadraticPadeJastrowKernel(JastrowKernelElectronNucleiBase):
    ...
    def __init__(self, nup: int, ndown: int, atomic_pos: torch.Tensor, cuda: bool,
        ...
        self.rweight = nn.Parameter(torch.as_tensor([a]), requires_grad=True).to(self.device)
        self.r2weight = nn.Parameter(torch.as_tensor([b]), requires_grad=True).to(self.device)
        self.weight = nn.Parameter(torch.as_tensor([c]), requires_grad=True).to(self.device)
