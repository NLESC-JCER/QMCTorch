...
class CalculatorAMS(CalculatorBase):
    ...
    def run(self):
        ...
        rkf_path = os.path.join(
            plams_wd, os.path.join(wd, rkf_name))

        # configure plams and run the calculation
        self.init_plams()
        mol = self.get_plams_molecule()
        sett = self.get_plams_settings()
        job = plams.AMSJob(molecule=mol, settings=sett, name=wd)
        job.run()

        # extract the data to hdf5
        basis = self.get_basis_data(rkf_path)

        # remove adf data
        if self.savefile:
            shutil.copyfile(rkf_path, rkf_name)
            self.savefile = rkf_name
