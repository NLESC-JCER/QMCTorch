...
class H2(NEURAL_WF_BASE):
    ...
    def nuclear_repulsion(self):
        c0 = self.ao.atom_coords[0,:]
        c1 = self.ao.atom_coords[1,:]
...
def pos_curve(net,X = np.linspace(0.2,1.5,11)):
    ...
    for x in X:
        ...
        net.wf.ao.atom_coords.data[0,2] = -x
        net.wf.ao.atom_coords.data[1,2] = x
...
def single_point(net,x=0.69,loss='variance',alpha=0.01):
    ...
    net.wf.ao.atom_coords.data[0,2] = -x
    net.wf.ao.atom_coords.data[1,2] = x
    net.wf.ao.atom_coords.requires_grad = True
    ...
    for i in range(2):
        x,y,z = net.wf.ao.atom_coords.data[i,:].numpy()
        u,v,w = net.wf.ao.atom_coords.grad.data[i,:].numpy()
        ax.quiver(x,y,z,-u,-v,-w,color='black',length=1.,normalize=True,pivot='middle')

    plt.show()


def geo_opt(net,x=1.25,sigma=1.24,loss='energy'):
    ...
    net.wf.ao.atom_coords.requires_grad = True
    net.wf.ao.bas_exp.requires_grad = False
...
def mo_opt(net,x=0.69,sigma=1.24,loss='variance'):
...
if __name__ == "__main__":
    ...
    # define the molecule
    #mol = Molecule(atom='H 0 0 -0.37; H 0 0 0.37',basis='dz')
