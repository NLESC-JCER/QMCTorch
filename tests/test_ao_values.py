...
class TestAOvalues(unittest.TestCase):
    ...
    def test_ao(self):
        ...
        aovals_ref = self.m.eval_gto('GTOval_cart',self.pos.detach().numpy()[:,:3])
    ...
    def test_ao_deriv(self):
        ...
        ip_aovals_ref = self.m.eval_gto('GTOval_ip_cart',self.pos.detach().numpy()[:,:3])
        ip_aovals_ref = ip_aovals_ref.sum(0)

        assert np.allclose(ip_aovals[:,0,self.iorb],ip_aovals_ref[:,self.iorb])
