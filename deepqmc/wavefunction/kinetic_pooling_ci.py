...
class KineticPoolingCI(nn.Module):
    ...
    def forward(self, MO, d2MO, dJdMO=None, d2JMO=None):
        ...
        out = (btrace(iAup@Bup) + btrace(iAdown@Bdown)) * \
            torch.det(Adown) * torch.det(Aup)

        return -0.5*out.transpose(0, 1), (torch.det(Aup) * torch.det(Adown)).transpose(0, 1)
